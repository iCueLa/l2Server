...
internal abstract class CombatBehavior(val player: Player, var combatPreferences: CombatPreferences) :
    ...
    private suspend fun handleDeath() {
        ...
        if(player.isDead){
            if(player.isInsideZone(ZoneId.CHANGE_PVP_ZONE)) {
                    ...
                    resetTarget()
                    player.doRevive()
                    player.teleportTo(RandomZoneManager.getInstance().currentZone.loc, 20)
    ...
    protected open fun handleShots() {
        if(player is Autobot) {
            if (player.inventory.getItemByItemId(getShotId(player)) != null) {
                ...
                if(!player.inventory.getItemByItemId(getShotId(player)).item.isInfinity){
                ...
                player.inventory.addItem("", getShotId(player), 1, player, null)  //from 500 to 1 cause of automatic potions
            }
        }        
    ...
    override val consumables: List<Consumable> = mutableListOf(
        ...
        Consumable(728) { autobot -> autobot.getCombatBehavior().combatPreferences.useManaPots && autobot !is SecretManaRegen && autobot.getManaPercentage() < 80 },
        Consumable(1539) { autobot ->
            autobot.getCombatBehavior().combatPreferences.useGreaterHealingPots && autobot.getHealthPercentage() < 90 && !autobot.hasEffect(
                2037
            )
        },
        Consumable(5592) { autobot -> autobot.getCombatBehavior().combatPreferences.useGreaterCpPots && autobot.getCpPercentage() < 85 }
    )
