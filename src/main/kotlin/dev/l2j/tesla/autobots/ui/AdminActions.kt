...
internal object AdminActions {
    ...
    //custom removed internal
    fun despawnAutobot(autobot: Autobot?) { //internal
        autobot?.despawn()
    }

    fun despawnAutobotCustom(autobot: Autobot?) {
        autobot?.despawnCustom()
    }

    //custom
    fun spawnit(player : Player) {
        val autobot = AutobotsDao.loadByName(player.name) ?: return
        runBlocking {
