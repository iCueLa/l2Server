...
internal object AutobotsDao {
 ...
 //       val exMember: ClanMember? = clan.getClanMember(autobot.objectId) ?: return
    ...
    private const val idExists = "SELECT count(1) from autobots where obj_Id=? LIMIT 1"
    fun botWithNameExists22( bid: Int) : Boolean{
        L2DatabaseFactory.getInstance().connection.use { con ->
            con.prepareStatement(idExists).use { ps ->
                ps.setInt(1, bid)
                ps.executeQuery().use { rset ->
                    rset.next()
                    return rset.getInt(1) == 1
    ...
    private const val idExists1 = "SELECT count(1) from autobots where obj_Id=? LIMIT 1"
    fun itemExist( bid: Int) : Boolean{
        L2DatabaseFactory.getInstance().connection.use { con ->
            con.prepareStatement(idExists1).use { ps ->
                ps.setInt(1, bid)
                ps.executeQuery().use { rset ->
                    rset.next()
                    return rset.getInt(1) == 1
