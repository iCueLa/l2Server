...
public class HWIDManager
{
	...
	public static void updateHWIDInfo(GameClient client, int windowsCount, HWIDInfoList.LockType lockType)
	{
		...
		hInfo.setIp(client.getConnection().getInetAddress().getHostAddress());
		...
		if (isFound)
		{
			try
			{
				...
				try
				{
					PreparedStatement statement = con.prepareStatement("UPDATE hwid_info SET WindowsCount=?,Account=?,PlayerID=?,LockType=?,IP=? WHERE HWID=?");
					...
					statement.setString(6, client.getConnection().getInetAddress().getHostAddress());
					statement.execute();
				}
				catch (Throwable localThrowable1)
				{
		...
		else
		{
			try
			{
				...
				try
				{
					PreparedStatement statement = con.prepareStatement("INSERT INTO hwid_info (HWID, WindowsCount, Account, PlayerID, LockType, IP) values (?,?,?,?,?,?)");
