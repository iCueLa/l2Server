...
public class SkillTreeData implements IXmlReader
{
	...
	//Custom Enchant SKill for donate Shop
	public List<EnchantSkillNode> getEnchantSkillsForCustom(Player player)
	{
		final List<EnchantSkillNode> result = new ArrayList<>();
...
//			if (skill != null && ((skill.getLevel() == SkillTable.getInstance().getMaxLevel(skill.getId()) && (esn.getValue() <= 112 || esn.getValue() <= 152)) || (skill.getLevel() == esn.getValue() - 1)))
			if (skill != null && ((skill.getLevel() == SkillTable.getInstance().getMaxLevel(skill.getId()) && (esn.getValue() == 101 || esn.getValue() == 141)) || (skill.getLevel() == esn.getValue() - 1)))
				result.add(esn);
		}
		return result;
	...
	public EnchantSkillNode getEnchantSkillData(int enchant)
	{
