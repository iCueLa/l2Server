...
import Customs.Events.Dungeon.DungeonStage;
...
//		PvpColor.checkColorPlayer(this);
	...
	public void useEquippableItem(ItemInstance item, boolean abortAttack)
	{
		...
		else
		{
			...
			//custom 
			if(PlayerMemoCustom.getVar(this, "delete_temp_item_"+item.getObjectId()) != null)
				sendMessage("Your "+item.getName()+" expires in "+MathUtil.convertMillisToTime(PlayerMemoCustom.getVarTimeToExpireSQL(this, "delete_temp_item_"+item.getObjectId())-System.currentTimeMillis())+".");
...
//		System.out.println("logout" + client.isDetached() + " pl " + gethwid());
			...
			PreparedStatement ps = con.prepareStatement(RESTORE_CHARACTER))
		{
			...
			try (ResultSet rs = ps.executeQuery())
			{
				while (rs.next())
				{
					...
					//custom playermemocustom
					PlayerMemoCustom.loadVariables(player);
					//restoreCustom(player,objectId,rs);
...
//			return true;
	...
	public boolean checkPvpSkill(WorldObject target, L2Skill skill)
	{
		...
		if (skill.isDebuff() || skill.isOffensive())
		{
			...
			//custom hit team player ( couldnt use skills on my self with that )
...
//			System.out.println("gere!!");
			...
			//			|| ( (skill.getSkillType().equals(L2SkillType.CHARGEDAM) || skill.isAOE()) && isCtrlPressed ) ) {
				if ( skill.getEffectRange() > 0 && isCtrlPressed && getTarget() == target
						&& (skill.isDamage() || skill.isAOE() || skill.getSkillType() == L2SkillType.CHARGEDAM ) ){
//					System.out.println("checkpvpskill aoe ctr no flag");
					return true;
				}
				return false;
			}
	...
	public void setHero(boolean hero)
	{
		...
		//custom hero skills on sub classes
		if (hero){
			for (L2Skill skill : SkillTable.getHeroSkills())
				addSkill(skill, false);
		}
...
//			VoteManagerTask.getInstance().add(this);
	...
	private DungeonStage dungeon_stage = null;

	public void setDungeonStage(DungeonStage currentStage)
	{
		dungeon_stage = currentStage;
	}
	
	public DungeonStage getDungeonStage()
	{
		return dungeon_stage;
	}
	
	
	private final Map<String, PlayerVar> _varsCustom = new ConcurrentHashMap<>();
	/**
	 * @return player memos.
	 */
	...
	public int getTitleColorVip() {
		...
		//return Integer.decode("0x" + _originalTitleColorVip + "88AA88" );
	}
	
	public int getNameColorVip() {
		return _originalNameColorVip;
//		return Integer.decode("0x" + _originalNameColorVip + "88AA88" );		
	}
