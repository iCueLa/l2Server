...
public class ShortcutList
{
	...
	//custom
	//private static final String LOAD_SHORTCUTS1 = "SELECT char_obj_id, type, id, level,slot,page FROM character_shortcuts WHERE char_obj_id=? AND class_index=? AND id=?";
//	private static final String REPLACE = "REPLACE INTO character_shortcuts (char_obj_id,type,id,level,slot,page) values(?,?,?,?,?,?)";
		...
		if (checkIntegrity)
		{
			switch (shortcut.getType())
			{
				case ITEM:
					...
					//custom not working
				//	if(item.isAugmented()) {
//						System.out.println("augm:" + item.getAugmentation().getAugmentationId());
				//		shortcut.setAugm(item.getAugmentation().getAugmentationId());
				//	}
					break;
...
//custom
/*	public void updateDp(int id, int level, ShortcutType type)
	{
		
		try (Connection con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement ps = con.prepareStatement(LOAD_SHORTCUTS1))
		{
			ps.setInt(1, _owner.getObjectId());
			ps.setInt(2, _owner.getClassIndex());
			ps.setInt(3, id);
			...
			try (ResultSet rs = ps.executeQuery())
			{
				while (rs.next())
				{
					PreparedStatement pss = con.prepareStatement(REPLACE);
					pss.setInt(1, _owner.getObjectId());
					pss.setString(2, type.toString());
					pss.setInt(3, id);
					pss.setInt(4, level);
					pss.setInt(5,rs.getInt("slot"));
					pss.setInt(6,rs.getInt("page"));
