 ...
 */
public final class Weapon_def_with_mo388 extends Item
{
	 ...
	 */
	public Weapon_def_with_mo388(StatsSet set)
	{
		super(set);
	...
	public List<L2Effect> getSkillEffects(Creature caster, Creature target, boolean crit)
	{
		...
		final L2Skill skillOnCrit = _skillsOnCrit.getSkill();
		if (skillOnCrit == null)
			return Collections.emptyList();
		...
		if (_skillsOnCritCondition != null && !_skillsOnCritCondition.test(caster, target, skillOnCrit))
			return Collections.emptyList();
		...
		for (L2Effect e : _skillsOnCrit.getSkill().getEffects(caster, target, shld, false))
			effects.add(e);
