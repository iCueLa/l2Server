...
//					if (obj == this)
	...
	//custom  ,to allaxa gia na valw to protect wh enchant
	public final <A> List<A> getKnownTypeRadius(int radius)
	{
		final WorldRegion region = _region;
		if (region == null)
			return Collections.emptyList();
		...
		for (WorldRegion reg : region.getSurroundingRegions())
		{
			for (WorldObject obj : reg.getObjects())
			{
				if (obj == this ||  !Util.checkIfInRange(radius, this, obj, true))
					continue;
