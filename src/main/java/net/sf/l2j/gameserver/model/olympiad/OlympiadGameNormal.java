...
import Customs.mods.olyResetSkills;
...
abstract public class OlympiadGameNormal extends AbstractOlympiadGame
{
	...
	protected final boolean portPlayersToArena(List<Location> spawns)
	{
		...
		//customs
			for(L2Skill sk : _playerOne.getPlayer().getSkills().values()){
				if(sk.getId() >= 10050 && sk.getId() <= 10119){
					_playerOne.getPlayer().removeSkill(sk.getId(), false);
					_playerOne.getPlayer().addSkill(sk, true);
					_playerOne.getPlayer().sendSkillList();
				}
			}
			for(L2Skill sk : _playerTwo.getPlayer().getSkills().values()){
				if(sk.getId() >= 10050 && sk.getId() <= 10119){
					_playerTwo.getPlayer().removeSkill(sk.getId(), false);
					_playerTwo.getPlayer().addSkill(sk, true);
					_playerTwo.getPlayer().sendSkillList();
				}
			}

		//custom , Resetting skill timers before match start excluding a few that shouldn't be reseted 
		try {
			for (Timestamp sk : _playerOne.getPlayer().getReuseTimeStamps()) {
				if (sk.hasNotPassed() && olyResetSkills.ResetSkillAtOly(sk.getSkillId())) {
					L2Skill skill = null;
					try
					{
						skill = SkillTable.getInstance().getInfo(sk.getSkillId(), sk.getSkillLvl());
					}
					catch (Exception e)
					{
					}
					...
					if (skill != null)
					{
						_playerOne.getPlayer().enableSkill(skill); 
					}
				}
			}
			_playerOne.getPlayer().sendPacket(new SkillCoolTime(_playerOne.getPlayer()));
		}
		catch (Exception e) {
			e.printStackTrace();
		}
		...
		try {
			for (Timestamp sk : _playerTwo.getPlayer().getReuseTimeStamps()) {
				if (sk.hasNotPassed() && sk.getReuse()/1000 < 1260) {
					L2Skill skill = null;
					try
					{
						skill = SkillTable.getInstance().getInfo(sk.getSkillId(), sk.getSkillLvl());
					...
					if (skill != null)
					{
						_playerTwo.getPlayer().enableSkill(skill); 
					}
				}
			}
			_playerTwo.getPlayer().sendPacket(new SkillCoolTime(_playerTwo.getPlayer()));
		}
		catch (Exception  e) {
			e.printStackTrace();
		}
	...
	protected final boolean checkBattleStatus()
	{
		...
		//custom  ,anti dual box
		if (!CustomConfig.ALT_OLY_ALLOW_DUALBOX_OLY)
		{
			...
			/*	String ip1 = _playerOne.getPlayer().getClient().getConnection().getInetAddress().getHostAddress();
				...
				if (ip1.equals(ip2))
				{
					...
					System.out.println("OLYMPIAD: " + _playerOne.getPlayer().getName() + " and " + _playerTwo.getPlayer().getName() + ". SAME IP.");
					return false;
				}*/
				String hwid1 = _playerOne.getPlayer().gethwid();
				String hwid2 = _playerTwo.getPlayer().gethwid();
	...
	{
		//custom , instant update status of points
