...
public class VillageMaster extends Folk
{
	...
	public void onBypassFeedback(Player player, String command)
	{
		...
		else if (actualCommand.equalsIgnoreCase("change_clan_leader"))
		{
			...
			if (clan.getNewLeaderId() == 0)
			{
				...
				//custom instantly change paty leader
		...
		else if (command.startsWith("Subclass"))
		{
			...
			/*		if (player.getSubClasses().size() >= 3)
					...
					*/
					...
					//custom  , max number of subs' per player
					if (player.getSubClasses().size() >= CustomConfig.ALLOWED_SUBCLASS)
					{
						html.setFile("data/html/villagemaster/SubClass_Fail.htm");
						player.sendMessage("You reached maximum number of subclasses.");
						break;
					}
				...
				case 4: // Add Subclass - Action (Subclass 4 x[x])
					...
					if (allowAddition && isValidNewSubClass(player, paramOne))
					{
						...
						//custom remove weap/armor on sub change
						if (!ItemRestriction(player))
							return;// Check the player for items during subclass..to avoid bugs

						//custom
						if(player.isDressMeEnabled() && player.getDressMeData().getChestId() != 0 ) {
							player.getDressMeData().reset();
				...
				case 6: // Change/Cancel Subclass - Choice
					// validity check
					if (paramOne < 1 || paramOne > CustomConfig.ALLOWED_SUBCLASS)
						return;
	...
	{
		//custom  ,subclass everywhere
		if (CustomConfig.ALT_GAME_SUBCLASS_EVERYWHERE)
			 return true;
	...
	//custom remove item on sub
	private static boolean ItemRestriction(Player player)
	{
		if (player.getActiveWeaponInstance() != null || player.getActiveArmorItem() != null || player.getActiveLegItem() != null)
		{
			for (ItemInstance items : player.getInventory().getItems())
			{
				if (items.isEquipped() && (items.equals(player.getInventory().getPaperdollItem(7)) || items.equals(player.getInventory().getPaperdollItem(8))
					|| items.equals(player.getInventory().getPaperdollItem(10)) || items.equals(player.getInventory().getPaperdollItem(11))))
