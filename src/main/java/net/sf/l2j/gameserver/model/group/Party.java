...
public class Party extends AbstractGroup
{
	...
	//		disband();
		//custom dont destroy party if >2 member but give it to other player
		if (isLeader(player))
		{
			if(_members.size() == 2)
				disband();
			...
			{
				//TODO Change leader to another player and do not destroy the party
				//disband();
				removePlayerFromPartyAndGiveParty(player, type);
			}
		}
		else if(_members.size() == 2)
		{
			disband();
		}
		else
		{
			//custom if in party zone 
			checkPartyFarmLeader(player);
	...
	public void checkPartyFarmLeader(Player player){
		if(PartyFarm.getInstance().isStarted() ){
			if(player.isInsideZone(ZoneId.PARTY)){
				player.teleportTo(82635, 148798, -3464, 1);
				player.sendMessage("You should not leave your party in the Party Farm.");
			}
		}
	}
	public void checkPartyFarmMembers(Player player){
		if(_members.size() < CustomConfig.MINIMUM_PLAYERS_IN_PARTY_ZONE){
			for(Player p : _members){
				if(p.isInsideZone(ZoneId.PARTY)){
					p.teleportTo(82635, 148798, -3464, 1);
					p.sendMessage("You must be at least 3 people in the party to stay,make your party and try again.");
				}
			}
		}
	}
	...
	public void removePlayerFromPartyAndGiveParty(Player player, MessageType type)
	{
		//custom if in party zone 
		checkPartyFarmLeader(player);
		...
		// Change leader to another player
		for(Player member : _members)
		{
			if(member == null)
				continue;
			
			changePartyLeader(member.getName());
			break;
		}
	}
