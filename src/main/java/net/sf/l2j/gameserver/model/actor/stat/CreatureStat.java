...
public class CreatureStat
{
	...
	public int getCustomModifier(Creature target, L2Skill skill)
	{
		double attack = _activeChar.getTemplate().getBaseMAtk() * ((_activeChar.isChampion()) ? Config.CHAMPION_ATK : 1);
		...
		// Calculate modifiers Magic Attack
		return (int) calcStat(Stats.SKILL_CHANCE, attack, target, skill);
	}
...
//		return (int) calcStat(Stats.MAGIC_DEFENCE, _activeChar.getTemplate().getBaseMDef() * ((_activeChar.isRaidRelated()) ? Config.RAID_DEFENCE_MULTIPLIER : 1), target, skill);
		int val = (int) calcStat(Stats.MAGIC_DEFENCE, _activeChar.getTemplate().getBaseMDef() * ((_activeChar.isRaidRelated()) ? Config.RAID_DEFENCE_MULTIPLIER : 1), target, skill);
...
//		return (int) calcStat(Stats.POWER_DEFENCE, _activeChar.getTemplate().getBasePDef() * ((_activeChar.isRaidRelated()) ? Config.RAID_DEFENCE_MULTIPLIER : 1), target, null);
		int val = (int) calcStat(Stats.POWER_DEFENCE, _activeChar.getTemplate().getBasePDef() * ((_activeChar.isRaidRelated()) ? Config.RAID_DEFENCE_MULTIPLIER : 1), target, null);
		if (_activeChar instanceof Player && ((Player) _activeChar).getClassId().getId() >= 88)
