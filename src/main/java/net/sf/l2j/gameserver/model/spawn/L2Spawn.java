...
public final class L2Spawn implements Runnable
{
	...
	public int calculateRespawnTime()
	{
		...
		//custom respawn delay based on online players
		
	/*	int count=World.getInstance().getPlayers().size();
		if (count<=30)
			respawnTime*=1.15;//+15% re-spawn delay
		else if (count<=80)
			respawnTime*=1;//actual re-spawn delay
		else if (count<=120)
			respawnTime*=0.90;//-10% re-spawn delay
		else if (count<=200)
			respawnTime*=0.80;//-20% re-spawn delay
	...
	/**
	 * Custom respawn for corpse life drain skill fix
	 */
	public void doCustomRespawn()
	{
		// Check if respawn is possible to prevent multiple respawning caused by lag
		if (_respawnEnabled)
		{
			// Calculate the random time, if any.
			final int respawnTime = (calculateRespawnTime() * 1000) / 2;
