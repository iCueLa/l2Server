...
public class RaidBoss extends Monster
{
	...
	//custom rb respawn
	private ScheduledFuture<?> _checkRange;
	...
	public void onSpawn()
	{
		...
		_checkRange = ThreadPool.scheduleAtFixedRate(() ->
		{
			...
			{
				if (!isInsideRadius(spawn.getLocX(), spawn.getLocY(), spawn.getLocZ(), CustomConfig.RB_RETURN_SPAWN_RAD , true, false))
					teleportTo(spawn.getLocX(), spawn.getLocY(), spawn.getLocZ(), 0);
			}
			...
			if (!isDead())
			{
				...
				if (!isInCombat())
				{
					...
					//		return;
						...
						if (!isInsideRadius(spawn.getLocX(), spawn.getLocY(), spawn.getLocZ(), Math.max(CustomConfig.RB_RETURN_SPAWN_RAD,400), true, false)) //Math.max(Config.MAX_DRIFT_RANGE, 300)
