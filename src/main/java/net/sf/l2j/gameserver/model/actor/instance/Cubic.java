...
import Customs.Events.CTF.CTFEventTeam;
import Customs.Events.DM.DMEvent;
import Customs.Events.TvT.TvTEvent;
import Customs.Events.TvT.TvTEventTeam;
...
public class Cubic
{
	...
	public void getCubicTarget()
	{
		...
		//custom events
	      ...
	      if (CTFEvent.isStarted() && CTFEvent.isPlayerParticipant(this._owner.getObjectId())) {
            final CTFEventTeam enemyTeam = CTFEvent.getParticipantEnemyTeam(this._owner.getObjectId());
        ...
        if (DMEvent.isStarted() && DMEvent.isPlayerParticipant(this._owner.getObjectId())) {
            if (ownerTarget.getActingPlayer() != null) {
                final Player target2 = ownerTarget.getActingPlayer();
                if (!target2.isDead()) {
                    this._target = (Creature)ownerTarget;
                }
            }
    ...
    /*    if (LMEvent.isStarted() && LMEvent.isPlayerParticipant(this._owner.getObjectId())) {
            if (ownerTarget.getActingPlayer() != null) {
                final Player target2 = ownerTarget.getActingPlayer();
                if (!target2.isDead()) {
                    this._target = (Creature)ownerTarget;
        ...
        if (TvTEvent.isStarted() && TvTEvent.isPlayerParticipant(this._owner.getObjectId())) {
            final TvTEventTeam enemyTeam2 = TvTEvent.getParticipantEnemyTeam(this._owner.getObjectId());
            if (ownerTarget.getActingPlayer() != null) {
                final Player target = ownerTarget.getActingPlayer();
                if (enemyTeam2.containsPlayer(target.getObjectId()) && !target.isDead()) {
                    this._target = (Creature)ownerTarget;
                }
            }
            return;
