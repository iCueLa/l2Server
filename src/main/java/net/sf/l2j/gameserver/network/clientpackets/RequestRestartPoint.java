...
import net.sf.l2j.gameserver.model.zone.type.PeaceIHZone;
...
public final class RequestRestartPoint extends L2GameClientPacket
{
	...
	private void portPlayer(Player player)
	{
		...
		// Nothing has been found, use regular "To town" behavior.
			...
			final PeaceIHZone helper = ZoneManager.getInstance().getZone(player, PeaceIHZone.class);
			if (helper != null && helper.getId() == 110021)
				loc = helper.getLoc();
			else
				loc = MapRegionData.getInstance().getLocationToTeleport(player, TeleportType.TOWN);
		}
