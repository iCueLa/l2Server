...
public final class CharacterCreate extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		}while(AutobotsDao.INSTANCE.botWithNameExists22(id));


		final Player player = Player.create(id, template, getClient().getAccountName(), _name, _hairStyle, _hairColor, _face, Sex.values()[_sex]);
		if (player == null)
		{
			sendPacket(CharCreateFail.REASON_CREATION_FAILED);
			return;
		...
		// Set default values.
//		player.setCurrentCp(player.getTemplate().getBaseCpMax(player.getLevel()));
...
//		player.getPosition().set(template.getRandomSpawn());
		...
		//custom  , starting loc
	      if (!CustomConfig.ALLOW_CUSTOM_SPAWN_LOCATION)
	    	  player.getPosition().set(template.getRandomSpawn());
	      else
	    	  player.setXYZInvisible(CustomConfig.CUSTOM_SPAWN_LOCATION[0], CustomConfig.CUSTOM_SPAWN_LOCATION[1], CustomConfig.CUSTOM_SPAWN_LOCATION[2]);


		player.setTitle(CustomConfig.STARTING_TITLE);
		...
		//Custom level add adena
		player.getInventory().addItem("Init", 57, CustomConfig.STARTING_ADENA, player, null);
//		player.getStat().setLevel((byte)Experience.LEVEL[CustomConfig.STARTING_LVL]);
		player.addExpAndSp(Experience.LEVEL[CustomConfig.STARTING_LVL], 1000);
