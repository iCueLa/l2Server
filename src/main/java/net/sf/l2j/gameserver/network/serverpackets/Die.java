...
public class Die extends L2GameServerPacket
{
	...
	public Die(Creature cha)
	{
		...
		if (cha instanceof Player)
		{
			...
			//custom 
			/*(TvTEvent.isStarted() && TvTEvent.isPlayerParticipant(player.getObjectId())) 
			||  (CTFEvent.isStarted() && CTFEvent.isPlayerParticipant(player.getObjectId())) || (DMEvent.isStarted() && DMEvent.isPlayerParticipant(player.getObjectId()))*/
			_event = DungeonManager.getInstance().isInDungeon(player) || player.isInFunEvent() ||  player.isInTournament()  ;
			
			_canTeleport = player.isInsideZone(ZoneId.CHANGE_PVP_ZONE);
