...
public final class Say2 extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if(checkVipChat(player , _text , _type))
			return;
	...
	public static boolean checkVipChat(Player player , String text , int type) {
		if(player.isVip() ) {
			if(type == 0) {
				if(text.startsWith("^") && FloodProtectors.performAction(player.getClient(), Action.HERO_VOICE) && !player.isChatBanned() ) {
					World.toAllOnlinePlayers(new CreatureSay(0, Say2.HERO_VOICE ,"[VIP]: " + player.getName(),text.substring(1)) );
					return true;
				}
			}
		}
