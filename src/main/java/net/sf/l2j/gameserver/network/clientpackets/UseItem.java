...
public final class UseItem extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		//custom pvp zone use items
        if (activeChar.isInsideZone(ZoneId.CHANGE_PVP_ZONE) && !RandomZone.checkItem(item)){
        	activeChar.sendMessage("You cannot use " +item.getName()+ " inside this zone.");
        	return;
        }
      //custom olympiad use items
        if(activeChar.isInOlympiadMode() && item.getItem().isOlyRestrictedItem()){  //activeChar.isPlayingOlympiad()
			activeChar.sendMessage("You cannot use " +item.getName());
        	return;
        }
		...
		//custom cant use bsoe in events,pvpzone,event
		//TODO add for other events too
		if(  activeChar.isInFunEvent()	)
		{
			...
			if (sHolders != null)
			{
				for (IntIntHolder sHolder : sHolders)
				{
					final L2Skill skill = sHolder.getSkill();
					if (skill != null && (skill.getSkillType() == L2SkillType.TELEPORT || skill.getSkillType() == L2SkillType.RECALL)){
						activeChar.sendMessage("Cant use this item in this place.");
						return;
					}
				}
			}
