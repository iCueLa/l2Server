	...
	private void initialize()
	{
		...
		try (Connection con = L2DatabaseFactory.getInstance().getConnection())
		{
			try (Statement st = con.createStatement())
			{
				...
				try (ResultSet rs = st.executeQuery("SELECT ally_id FROM clan_data"))
				{
					while (rs.next()) {
						if(rs.getInt(1) != 0)
							usedObjectIds.add(rs.getInt(1));
				...
				try (ResultSet rs = st.executeQuery("SELECT crest_id FROM clan_data"))
				{
					while (rs.next()) {
						if(rs.getInt(1) != 0)
							usedObjectIds.add(rs.getInt(1));
				...
				try (ResultSet rs = st.executeQuery("SELECT crest_large_id FROM clan_data"))
				{
					while (rs.next()) {
						if(rs.getInt(1) != 0)
							usedObjectIds.add(rs.getInt(1));
				...
				try (ResultSet rs = st.executeQuery("SELECT ally_crest_id FROM clan_data")) {
					while (rs.next()) {
						if (rs.getInt(1) != 0)
							usedObjectIds.add(rs.getInt(1));
				...
				try (ResultSet rs = st.executeQuery("SELECT obj_Id FROM autobots"))
				{
					while (rs.next()){
						if(rs.getInt(1) != 0)
							usedObjectIds.add(rs.getInt(1));
