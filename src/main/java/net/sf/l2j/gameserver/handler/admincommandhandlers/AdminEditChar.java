...
public class AdminEditChar implements IAdminCommandHandler
{
	...
	private static void gatherCharacterInfo(Player activeChar, Player player, String filename)
	{
		...
		//custom
//		html.replace("%hwid%", player.gethwid() != null ? player.gethwid() : player.getIP() );
		html.replace("%hwid%", player instanceof  Autobot ? "Bot" : player.gethwid() != null ? player.gethwid() : player.getIP() );
...
//		html.replace("%ip%", (player.getClient().isDetached()) ? "Disconnected" : player.getClient().getConnection().getInetAddress().getHostAddress());
		html.replace("%ip%", (player.getClient().isDetached()) ? "Disconnected" : player instanceof Autobot ? "Bot" :  player.getClient().getConnection().getInetAddress().getHostAddress());
	...
	private static void findDualbox(Player activeChar, int multibox)
	{
		...
		{
			GameClient client = player.getClient();
			if (client == null || client.isDetached() || player instanceof Autobot)
				continue;
