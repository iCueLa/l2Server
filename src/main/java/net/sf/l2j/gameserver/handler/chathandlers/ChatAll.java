	...
	*/
	...
	public void handleChat(int type, Player activeChar, String params, String text)
	{
		    ...
		    if (text.startsWith("."))
		    {
		            ...
		            command1 = st.nextToken().substring(1);
	            	if(VoicedCommandHandler.getInstance().getHandler(command1) == null)
	            		return;
		        ...
		        if (!vcd_used)
		         {
		        	...
		        	String[] bannedWords = {"fail",  "fail server", "corruption", "corrupted", "sanity", "yola",  "server" , "fuck" };
					
					String t = text;
					for (String ban : bannedWords)
					{
						t = t.replaceAll(ban, "-.-");
						t = t.replaceAll(ban.toUpperCase(), "-.-");
					}
