...
public class FuncEnchant extends Func
{
	...
	public double calc(Creature effector, Creature effected, L2Skill skill, double base, double value)
	{
		...
		//Custom instance , oly max enchant is 6 (items keep the enchant but the status is as +6)
		int maxOlyEnchant = CustomConfig.OlympiadEnchant;
		if (effector != null && effector instanceof Player)
		{
			Player player = (Player)effector;
			if (player.isInOlympiadMode() && maxOlyEnchant >= 0 &&
					(enchant + overenchant) > maxOlyEnchant)
			{
				if (maxOlyEnchant > 3)
				{
					overenchant = maxOlyEnchant - 3;
				}
				else
				{
					overenchant = 0;
					enchant = maxOlyEnchant;
				}
			}
		}
