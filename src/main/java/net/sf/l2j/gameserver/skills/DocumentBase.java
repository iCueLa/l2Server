...
abstract class DocumentBase
{
	...
	protected void parseTemplate(Node n, Object template)
	{
		...
		for (; n != null; n = n.getNextSibling())
		{
			...
			//custom
			else if ("customand".equalsIgnoreCase(n.getNodeName()))
				attachFuncBalance(n, template, "customand", condition);
		}
	}
	
	//custom for skill balance(customand olympiad = true/false and stats inside them)
	//custom this will win time from mul stat then olympiad false , again and again.
	protected void attachFuncBalance(Node n, Object template, String function, Condition attachCond)
	{
		String sta = n.getAttributes().getNamedItem("olympiad").getNodeValue();
		boolean bool = false;
		if(sta.equalsIgnoreCase("true"))
			bool = true;
		
		Condition cond = null;
		cond = joinAnd(cond, new ConditionPlayerState(PlayerState.OLYMPIAD, bool));

		parseTemplateCustom(n,template,cond);
	}

	protected void parseTemplateCustom(Node n, Object template,Condition condition)
	{
		n = n.getFirstChild();
		if (n == null)
			return;
		...
		for (; n != null; n = n.getNextSibling())
		{
			...
			else if ("effect".equalsIgnoreCase(n.getNodeName()))
			{
				...
				attachFuncBalance(n, template, "and", condition);
		}
	}
	...
	protected void attachEffect(Node n, Object template, Condition attachCond)
	{
		...
		if (attrs.getNamedItem("time") != null){
			...
			//custom skill duration time for buffs
			 if(CustomConfig.ENABLE_MODIFY_SKILL_DURATION) {
				 if(CustomConfig.SKILL_DURATION_LIST.containsKey(((L2Skill) template).getId())){
					 if(((L2Skill) template).getLevel() < 100){
						time = CustomConfig.SKILL_DURATION_LIST.get(((L2Skill) template).getId());
					 }
