...
public final class Formulas
{
	...
	public static final double calcMagicDam(Creature attacker, Creature target, L2Skill skill, byte shld, boolean ss, boolean bss, boolean mcrit)
	{
		...
		if (Config.MAGIC_FAILURES && !calcMagicSuccess(attacker, target, skill))
		{
			...
			damage *= 2.1; //4 , 22/11 change from 2.5
	...
	public static final void calcLethalHit(Creature attacker, Creature target, L2Skill skill)
	{
		...
		if (skill.getLethalChance2() > 0 && Rnd.get(1000) < calcLethal(attacker, target, skill.getLethalChance2(), skill.getMagicLevel()))
		{
			...
			if(target instanceof RaidBoss || target instanceof DungeonMob || target instanceof Monster)
				return;
...
//	public static boolean calcEffectSuccess(Creature attacker, Creature target, EffectTemplate effect, L2Skill skill, byte shld, boolean bss)
	 ...
	 
	{
		...
		double rat = Math.max(1, Math.min((baseChance * statModifier * skillModifier * mAtkModifier * lvlModifier * customModifier * balancer), 99));
		...
		if (Config.DEVELOPER) {
			LOGGER.info("rat: " + rat);
			LOGGER.info("calcEffectSuccess(): name:{} eff.type:{} power:{} statMod:{} skillMod:{} mAtkMod:{} lvlMod:{} total:{}%  customMod:{}.", skill.getName(), type.toString(), baseChance, String.format("%1.2f", statModifier), String.format("%1.2f", skillModifier), String.format("%1.2f", mAtkModifier), String.format("%1.2f", lvlModifier), String.format("%1.2f", rate), customModifier);
		}
	...
	public static boolean calcSkillSuccess(Creature attacker, Creature target, L2Skill skill, byte shld, boolean bss)
	{
		...
		final double mAtkModifier = getMatkModifier(attacker, target, skill, bss);
		final double lvlModifier = getLevelModifier(attacker, target, skill);
		final double customModifier =  attacker.calcStat(Stats.SKILL_CHANCE, 1, null, null);
		final double rate = Math.max(1, Math.min((baseChance * statModifier * skillModifier * mAtkModifier * lvlModifier * customModifier), 99));
		
		if (Config.DEVELOPER)
			LOGGER.info("calcSkillSuccess(): name:{} type:{} power:{} statMod:{} skillMod:{} mAtkMod:{} lvlMod:{} total:{}% customMod:{}.", skill.getName(), skill.getSkillType().toString(), baseChance, String.format("%1.2f", statModifier), String.format("%1.2f", skillModifier), String.format("%1.2f", mAtkModifier), String.format("%1.2f", lvlModifier), String.format("%1.2f", rate),customModifier);
